...
async function main() {
  try {
    ...
    const tmp_path = import_node_path.default.join(import_node_os.default.tmpdir(), ".space-sync");
    core.debug(`Cloning to ${tmp_path} ...`);
    import_fs_extra.default.mkdirSync(tmp_path, { recursive: true });
    import_fs_extra.default.copySync(".", tmp_path, {
      filter: (src) => {
        if (src === ".git") {
          return false;
        }
    ...
    (0, import_child_process.execSync)("git init -b main", { cwd: tmp_path, stdio: "inherit" });
...
`;
    import_fs_extra.default.writeFileSync(
      import_node_path.default.join(tmp_path, "README.md"),
      config + import_fs_extra.default.readFileSync(import_node_path.default.join(tmp_path, "README.md"))
    );
    core.debug(`Committing and pushing ...`);
    (0, import_child_process.execSync)("git add .", { cwd: tmp_path, stdio: "inherit" });
    (0, import_child_process.execSync)("git commit -m 'Sync to HuggingFace Spaces'", { cwd: tmp_path, stdio: "inherit" });
    (0, import_child_process.execSync)(
